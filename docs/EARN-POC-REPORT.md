...
## 4. Phase 3 — V0.5 候選方案技術驗證(2026-05-01 完成)

繼 Phase 1.5 multi-platform scanner 之後,鎖定 V0.5 方向(Bitfinex 70% + AAVE 30%),
在進入正式開發前驗證兩件最關鍵的技術:

### 4.1 PoC #1 — AAVE V3 Polygon 鏈上讀取

**目標**:不靠 web3.py,純 raw JSON-RPC eth_call 讀 AAVE V3 Pool 的 USDT supply rate,證明 ABI decode 正確、跟 DefiLlama 對齊。

**腳本**: `apps/api/scripts/poc_aave_polygon.py`

**結果**:

| 項目 | 數值 |
|---|---|
| RPC 用的 endpoint | `https://polygon.drpc.org`(免 API key,4 個 fallback 都行) |
| `currentLiquidityRate` (ray) | 37,645,681,454,172,136,323,455,593 |
| 換算 APR | 3.7646% |
| 換算 APY (per-second compound) | 3.8363% |
| aToken 地址驗證 | ✓ 與 hardcoded `0x6ab707...` 一致 |
| aToken totalSupply | 32,194,196 USDT(= 該 reserve TVL) |
| DefiLlama 對齊 | USDT0 base 3.7639%,差距 0.07%(✓ ACCEPTABLE) |

**關鍵發現** ⚠️:
- Polygon 的 USDT 有兩種:**PoS bridged USDT**(`0xc2132D05...`,我們讀的)和 **USDT0**(LayerZero/Tether 官方原生)
- AAVE V3 兩個都收,但 DefiLlama 主推 USDT0
- **production V0.5 要決定支援哪個**(我們可以兩個都接,但要決定 deposit/withdraw 預設用哪個)

**淨值計算**:
- gross APY 3.84%
- 扣 15% perf fee → **net 3.20%**
- vs 台灣定存 1.6% → ✓ 贏

### 4.2 PoC #2 — Bitfinex Funding API authenticated read

**目標**:用 .env 裡的 read-only API key 走 HMAC-SHA384 認證,讀帳戶 / wallets / funding offers / FRR,證明 V0.5 可以 talk to Bitfinex。

**腳本**: `apps/api/scripts/poc_bitfinex_funding.py`

**Bitfinex API key 設定**(讀的權限):
- ✓ Account Info / History / Orders / Margin Trading / Funding (read) / Wallets (read) / Settings
- ✗ 所有 write 權限關閉(production 才會打開 Funding offer/cancel + Wallets transfer)

**結果**:

| Probe | 結果 |
|---|---|
| `/v2/auth/r/info/user` | ✓ user_id, email, username, verified, created |
| `/v2/auth/r/wallets` | ✓ 讀 OK(空,我還沒入金) |
| `/v2/auth/r/funding/offers/fUSDT` | ✓ 讀 OK(空) |
| `/v2/auth/r/funding/credits/fUSDT` | ✓ 讀 OK(空) |
| `/v2/ticker/fUST`(public)| ✓ FRR / Bid / Ask / Last 全拿到 |

**關鍵踩雷** ⚠️:
- Bitfinex public API **用 `fUST` 表示 Tether USDT**(歷史遺留,UST 是 Tether 早年的代號)
- `/v2/ticker/fUSDT` 回 500 / 空 array,`/v2/ticker/fUST` 才是正確端點
- 但 auth endpoints `funding/offers/fUSDT` 跟 `funding/credits/fUSDT` 兩個 symbol 都接受
- 這個踩雷已修進 `poc_yield_scanner.py`(原本 fUSDT 抓不到,改 fUST 後正常)

**Live 市場數值(2026-05-01)**:

| 指標 | 日利率 | 年化 |
|---|---|---|
| FRR (即時撮合最高利率) | 0.0251% | **9.16% APY** |
| Bid (借方願付最高) | 0.0219% | 8.00% APY |
| Ask (貸方願收最低) | 0.0134% | 4.90% APY |
| **Last 成交(實務 baseline)** | 0.0137% | **5.00% APY** |
| Available to lend at FRR | — | 126M USDT |

**真實預期**:V0.5 plan 假設 10% 偏樂觀,**實務 baseline 5-9%**(看 lock at FRR 還是讓 market match)。

### 4.3 V0.5 數字重估(用真實 chain + market 數據)

| 部位 | 比例 | APY (Last/實務) | 貢獻 |
|---|---|---|---|
| Bitfinex Funding USDT | 70% | 5.00% | 3.50% |
| AAVE V3 Polygon | 30% | 3.84% | 1.15% |
| **Blended gross** | 100% | — | **4.65%** |
| 扣 15% perf fee | — | — | **net 3.95%** |

如果跑得到 FRR(7-9%)而非 Last(5%):
- Blended gross ≈ `0.7 × 8% + 0.3 × 3.84%` = 6.75%
- net ≈ **5.74%**

→ 真實 net APY 估算區間:**3.95% — 5.74%**(原 plan 假設 7.2%,偏高)。

**break-even TVL 重估**:
- 月運營 $3,500 / year $42K
- perf fee yield: 0.7%(保守,Last)— 0.86%(FRR)
- break-even = $42K / 0.7% — 0.86% = **$4.9M – $6M**

→ 仍比 V1 multi-DeFi 的 $7.5M 低,但比原 V0.5 假設($3.3M)高 50%。

### 4.4 Phase 3 結論

- ✅ AAVE V3 Polygon 純 raw RPC 可以讀,不用加 web3.py dependency
- ✅ Bitfinex auth flow 走得通,read 全部 OK
- ✅ FRR / Last / Bid-Ask 即時數據可拿
- ✅ 實務 net APY 預期 4-6%,法律 OK 的話 V0.5 商業上可行
- ⚠️ Polygon USDT vs USDT0 雙資產問題,V0.5 要決定支援哪個
- ⚠️ Bitfinex 公開 API 有 fUSDT vs fUST 歷史遺留,踩雷一次

### 4.5 Phase 3 還沒做的事(留給正式 V0.5 開發)

- [ ] Bitfinex 入金煙霧測試(USDT-TRC20 從 Tron HOT 送 1 USDT 到 Bitfinex Funding wallet)
- [ ] Bitfinex submit / cancel funding offer(需要 write 權限,等律師 OK 才開)
- [ ] Bitfinex withdraw 回 Tron(需要 withdraw 權限,production 走人工)
- [ ] AAVE supply / withdraw write path(testnet 先做,需 EOA 私鑰)
- [x] ~~Bridge research~~ → 見 4.6
- [ ] 4 鏈水位 reconciliation logic
- [ ] StrategyManager 跨 strategy 的 lazy rebalance

### 4.6 PoC #3 — Cross-chain bridge mainnet quote scan

**目標**:不真的 bridge 任何錢,但 query 真實 production API 看哪些 bridge 支援 Tron→Polygon USDT,以及實際 fee / slippage / 時間。

**腳本**: `apps/api/scripts/poc_bridge_scanner.py`

**預設認知校正**:Tron↔EVM USDT 的 testnet bridge 基本不存在(沒有 LP 維護)。最有用的方式是直接打 mainnet quote API。

**結果**:

| Bridge | 支援 Tron? | 結論 |
|---|---|---|
| **Symbiosis** | ✅ chain id 728126428 | **唯一可用的 API 化方案** |
| Allbridge Core | ❌ 只 EVM (11 鏈) | Tron 要走 Allbridge Classic 但已 deprecated |
| deBridge DLN | ❌ srcChainId 無 Tron | 跳過 |

**Symbiosis 實際 quote(Tron USDT → Polygon USDT PoS)**:

| Input | Output | Loss | Loss% | Fee USD | Time | 路徑 |
|---|---|---|---|---|---|---|
| 100 | 99.30 | 0.70 | 0.70% | $0.50 | 15s | USDT@Tron → USDC.e@Polygon |
| 1,000 | 997.51 | 2.49 | **0.25%** | $0.50 | 15s | 同上 |
| 10,000 | 9,978.92 | 21.08 | 0.21% | $0.50 | 15s | 同上 |

**重要發現** ⚠️:
1. **Symbiosis 的 fee 是「fixed $0.50 + AMM slippage」**:fixed cost 在 $100 級別吃掉 0.5%,$1000 以上才划算
2. **Symbiosis 的 destination 是 USDC.e,不是 USDT**:然後在 Polygon 上 swap USDC.e → USDT,所以 slippage 0.2% 是 AMM 滑價(不是 bridge 費)
3. **USDT0 路線不可用**:Symbiosis 沒有 USDC.e ↔ USDT0 的 AMM 路由(Polygon 上 USDT0 流動性還小)
   → V0.5 必須用 PoS USDT 路線(就是我們 PoC #1 讀的那個 reserve)
4. **時間 estimatedTime=15(秒)**:看起來太快,production 要驗證(可能實際是分鐘)

**V0.5 production bridge 策略(基於 PoC 結論)**:

| 路徑 | 用途 | Fee | 時間 | 自動化 |
|---|---|---|---|---|
| 主:**Binance withdraw** | 大額(>$10K)、不急 | ~$1 | 1-3 hr | 半自動(API + KYC) |
| 備:**Symbiosis** | 小急額、Binance 維護時 | 0.25% on $1K | ~15s-15min | 全自動 API |
| 緊急:OKX withdraw | Binance 整個出事 | ~$1 | 1-3 hr | 同 Binance |

**V0.5 經濟學(用真實 bridge fee 算)**:

假設 Quiver 每週 rebalance 1 次,bridge $100K(平台 float):
- 用 Symbiosis: 0.21% × $100K = $210/週 = $10,920/年
- 用 Binance: ~$1/單,但要拆 5 筆避免單筆觸發紅旗 = $5/年
- **Binance 省 ~$11K/年**(假設 TVL $5M、perf fee $30-50K/年,這是 22-37% 的營運成本差)

→ **Binance 為主、Symbiosis 為自動化備援**是最合理的 V0.5 策略。

### 4.7 Phase 3 整體結論

PoC Phase 3 三個子任務驗證完成:

| 子任務 | 結論 | V0.5 阻塞? |
|---|---|---|
| #1 AAVE V3 Polygon read | ✓ 純 raw RPC 可讀,APY 與 DefiLlama 對齊 | 無 |
| #2 Bitfinex Funding API | ✓ Auth 通,讀 OK,FRR 拿到 | 無 |
| #3 Bridge research | ✓ Symbiosis 自動化可用,Binance 為主 | 無 |

**沒有任何技術阻塞 V0.5 開發**。下一步就是**律師會面**(法律是現在唯一的真實阻塞)。
...
# Phase 1.5 multi-platform scanner
docker compose exec -T -e PYTHONPATH=/app api \
  python /app/scripts/poc_yield_scanner.py

# Phase 3 #1 — AAVE V3 Polygon read
docker compose exec -T -e PYTHONPATH=/app api \
  python /app/scripts/poc_aave_polygon.py

# Phase 3 #2 — Bitfinex Funding authenticated read
# (需先在 .env 設定 BITFINEX_API_KEY / BITFINEX_API_SECRET)
docker compose exec -T -e PYTHONPATH=/app api \
  python /app/scripts/poc_bitfinex_funding.py

# Phase 3 #3 — Cross-chain bridge mainnet quote scanner
docker compose exec -T -e PYTHONPATH=/app api \
  python /app/scripts/poc_bridge_scanner.py
```
