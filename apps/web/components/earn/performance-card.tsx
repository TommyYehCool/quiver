...
const STRINGS: Record<Locale, PerfStrings> = {
  "zh-TW": {
    title: "策略表現",
    subtitle: "你的 ladder 真的賺到錢的證據 — 對照 FRR 基準利率,看 Quiver 多賺了多少。",
    weightedAprLabel: "加權平均 APR",
    vsFrrAbove: (delta) => `比 FRR 高 +${delta}%`,
    vsFrrBelow: (delta) => `比 FRR 低 ${delta}%`,
    vsFrrEqual: "與 FRR 持平",
    baselineFrr: (apr) => `市場 FRR 基準 ${apr}%`,
    totalInterest30d: "30 天累計利息",
    daysWithData: (n) => `${n} 天有資料`,
    spikeCaptured: "Spike 抓取",
    spikeNone: "目前無 spike 部位",
    spikeAmount: (usdt) => `共 $${usdt} 在 spike rate`,
    bestApr: "目前最高 tranche APR",
    bestAprNone: "尚無在借出部位",
    ladderLabel: "Ladder 已部署",
    ladderNone: "—",
    trendLabel: "30 天日利息走勢",
    trendEmpty: "等待第一筆 snapshot(每日 cron 產生)",
    noDataYet:
      "你的部位剛 set up,Quiver 還在收第一批資料。掛單後 1-2 天會看到第一筆每日結算,再過 7-10 天 sparkline 才有意思。",
  },
  en: {
  ...
  ja: {
    title: "戦略パフォーマンス",
    subtitle: "ラダーが実際に収益を上げている証拠 — FRR ベースライトと比較して、Quiver がどれだけ市場を上回ったかを表示。",
    weightedAprLabel: "加重平均 APR",
    vsFrrAbove: (delta) => `FRR より +${delta}% 高い`,
    vsFrrBelow: (delta) => `FRR より ${delta}% 低い`,
    vsFrrEqual: "FRR と同等",
    baselineFrr: (apr) => `市場 FRR ベースライン ${apr}%`,
    totalInterest30d: "30 日累計利息",
    daysWithData: (n) => `${n} 日分のデータ`,
    spikeCaptured: "スパイク捕獲",
    spikeNone: "現在スパイクポジションなし",
    spikeAmount: (usdt) => `$${usdt} がスパイクレートで運用中`,
    bestApr: "現在最高 tranche APR",
    bestAprNone: "貸出ポジションなし",
    ladderLabel: "ラダー展開済み",
    ladderNone: "—",
    trendLabel: "30 日日次利息トレンド",
    trendEmpty: "初回スナップショット待ち(日次 cron)",
    noDataYet:
