...
const STRINGS: Record<Locale, Strings> = {
  "zh-TW": {
    ...
    s1: {
      h: "先講一個常見問題",
      body: '用戶 A 存了 $200 USDT、選平衡策略,結果在 Bitfinex 上只看到一筆「$200 全押、2 天、年化 2.92%」的掛單。他問:「我選了平衡的 5 階階梯,怎麼只看到 1 筆?」',
      bullets: [
        "答案:他的階梯**根本沒啟動**",
      ...
      insight:
        "這不是缺陷,是設計上的權衡。Bitfinex 平台規定每筆掛單最少 $150,如果硬切 5 階,小資金每階只有幾十塊,Bitfinex 會直接拒收。所以 Quiver 在資金不夠時會自動退回單一掛單。但介面沒告訴你這件事 — 這是個待修的使用體驗問題,已經紀錄在 backlog 的 F-5a-3.6。",
    },

    s2: {
      h: "階梯真的啟動時長什麼樣子?",
      lead: "假設你選平衡策略、有 $5,000、目前市場基礎年化 2.92%,Quiver 會把你的 $5,000 切成 5 階:",
      body: "每階用不同利率掛單。基礎那檔 1.0×(就是基礎利率)會最快被借走;後面幾階加價等「利率飆漲事件」(BTC 突然大漲大跌、有人爆倉急著借錢)。沒飆漲就掛在那邊不成交,等就是了。",
      columns: {
        pct: "佔比",
        amount: "金額",
        multiplier: "倍率",
      ...
      avgValue: "≈ 年化 3.41%",
      avgNote:
        "比單一掛單的 2.92% 多 0.49% — 而且這還沒算「飆漲真的發生時」後面幾階吃到 5-12% 的額外收益。階梯的真正價值在最後那兩階:平常掛在那邊不動,等到一個爆倉潮就直接吃到飆漲價、而且鎖長 7-14 天,等市場冷靜後你還在賺高利。",
    },
    ...
    s4: {
      ...
      items: [
        {
          title: "餘額 < $1,875",
          body: "選哪個策略類型結果都一樣 — 都走單一掛單。在冷市場(年化 < 5%)會掛 2 天、熱市場才會自動鎖長。要看到策略真正的差異,先想辦法讓餘額過門檻。",
        },
        {
          title: "餘額 $1,875 ~ $3,000",
          body: "**只有進取會啟動階梯**(因為門檻最低)。如果你想體驗 5 階階梯,選進取。但要知道:進取會把 35% 資金壓在 ≥ 1.5× 的飆漲檔,平常時段成交比較慢。",
        },
        {
          title: "餘額 $3,000 ~ $5,000",
          body: "保守和進取都能啟動階梯,平衡還不行。如果想要平衡的「6 / 2 / 1 / 0.7 / 0.3 五階」,需要再加碼到 $5,000。",
        },
        {
          title: "餘額 ≥ $5,000",
          body: "三個策略類型都能完整啟動階梯,可以照風險偏好挑。預設平衡就是最標準的選擇 — 6 成快進場、3.4 成等飆漲,最長鎖 30 天。",
        },
        {
          title: "市場很冷時(像現在 ~3% 年化)",
          body: "**不要為了「看起來在做事」而切到進取或鎖長**。冷市場切長期 = 把當下的爛價鎖住,等明天市場熱了你還是賺 2.92%。Quiver 預設 2 天期就是要在冷市場保留快速重新議價的彈性。",
        },
      ],
    },
      ...
      items: [
        {
          ...
          a: "不會。已成交的掛單會跑完當前期(2 ~ 30 天不等),到期回到 Bitfinex 的 Funding 錢包後,下一輪自動放貸才會用新策略計算。",
        },
        {
        ...
        },
        {
          ...
          a: "嚴格說是。但這是有意識的取捨 — 那幾檔是「等爆倉潮」用的保險。冷市場大部分時候不會中,但中一次抵很多次基礎檔的收益。如果你完全不想賭飆漲、想要每筆都成交,選保守(80% 在基礎、5% 在最高)。",
        },
        {
